...
class Pris:
    ...
    def ap_generate(self, te: TextEngine):
        if len(self._functions) or self.zdz or self.trip_ef or self.saon or self.achr or self.cb or self.terminal:
            ...
            if self.term_prot:
                te.ul('Отключение по внешнему сигналу от тепловой защиты (температурное реле) трансформатора;')
                te.ul('Контроль исправности (в том числе наличие питания) температурного реле трансформатора;')
            if self.cb:
