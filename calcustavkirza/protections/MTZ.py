    ...
    def calc_ust(self, te: TextEngine, res_sc_min: list, res_sc_max: list):
        ...
        if self.isz_prev:
            te.table_row(f'Первичный ток срабатывания защиты по условию согласования с предыдущей защитой '
                         ...
                         'Iсз ≥ Kн.с.·Iсз.пред.', f'{self.kns}*{self.isz_prev} ={self.kns*self.isz_prev:.2f}')
        ...
        if self.isz_posl:
            ...
            te.table_row('Коэффициент надёжности согласования с последующей защитой',
                         'Kн.с.', self.kns)
        isz_note = f'{self.isz_note} ' if self.isz_note else ''
        te.table_row(f'Принимаем первичный ток срабатывания защиты {isz_note}равным, А', 'Iсз', self.isz)
        ...
        if self.index_kz_min_res is not None:
            ...
            if self.i_kz_min_res:
                ...
                i_kz_min_res = None
        if i_kz_min_res:
            i_kz_min_res /= self.kt_res
        if i_kz_min_res and self.isz:
            ...
            te.table_row(f'Минимальный ток КЗ в конце зоны резервирования приведенный к напряжению места установки '
                         f'защиты, A', f'Iкзмин {self.i_kz_min_res_note}', f'{i_kz_min_res:.1f}')
            if k_ch_res < 1.3:
                self.add_warning(f'Коэффициент чувствительности в зоне резервирования мал ({k_ch:.2f})')
        if self.t is not None:
            t_note = f' {self.t_note}' if self.t_note else ''
            te.table_row(f'Время срабатывания защиты, с{t_note}', 'tср', self.t)
        if self.k:
