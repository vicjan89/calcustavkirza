...
class TODoc(Doc):
    ...
    def total(self, te: TextEngine):
        ...
        te.table_row(f'Время срабатывания защиты {self.t_note}, с', 'tср', self.t)
    ...
    def calc_ust(self, te: TextEngine, res_sc_min: list, res_sc_max: list):
        ...
        if self.i_kz_end_index is not None or self.i_kz_end is not None:
            if self.i_kz_end_index is not None:
                i_kz_max  = res_sc_max[str(self.i_kz_end_index)][1]
            else:
            ...
            if self.i_kz_end_index is not None and self.isz and i_kz_max > self.isz:
        ...
        if self.i_btn:
            i_btn = self.i_btn
        if self.sn:
            i_btn = self.sn / self.un / math.sqrt(3) * self.k_btn
        if i_btn:
            te.table_row('Ток срабатывания защиты по условию отстройки от броска тока намагничивания '
                         ...
                         f'{i_btn:.2f}')
            te.table_row('Коэффициент отстройки от броска тока намагничивания ', te.m(r'K_{BTH}'), self.k_btn)
        if self.sn:
            te.table_row('Суммарная мощность всех трансформаторов подключенных к защищаемому присоединению, кВА',
                         'Sn', self.sn)
        elif self.i_btn:
            te.table_row('Бросок тока намагничивания всех трансформаторов подключенных к зазищаемому присоединению, кВА',
                         'Iбтн', self.i_btn)
        if self.isz_posl:
            self.sogl_posl(te)
        if self.isz_prev:
        ...
        if i_kz_max_begin and self.isz:
...
class TO(Element, TODoc):
    '''
    i_kz_begin_index: str | int | None = None # индекс шины для проверки чувствительности
    i_kz_begin: int | None = None # ток КЗ в максимальном режиме в начале линии для проверки чувствительности
    i_kz_end_index: str | int | None = None # индекс шины для отстройки тока срабатывания
    ...
    i_kz_begin: int | None = None # ток КЗ в максимальном режиме в начале линии для проверки чувствительности
    i_kz_end_index: str | int | None = None # индекс шины для отстройки тока срабатывания
    i_kz_end: float | None = None # ток КЗ в максимальном режиме в конце линии для отстройки тока срабатывания
    i_kz_end_note: str = ''
    kn: float = 1.1
    Kns: float = 1.1
    kt: float = 1.
    ...
    k: int | None = None # коэффициент кривой обратнозависимой характеристики
    i_btn: float | None = None
    k_btn: float = 4. # коэффициент отстройки от бросков токов намагничивания
    sn: float | None = None # суммарная мощность трансформаторов в кВА
    un: float | None = None # номинальное напряжение сети
    name: str = 'Токовая отсечка'
    name_short: str = 'ТО'
