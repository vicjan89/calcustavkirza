...
class T3WPDIF(Element):
    ...
    Kots_ots: float = 1.2 #коэффициент отстройки для дифотсечки
    K_per: float
    Knb1: float = 0.65 #отношение амплитуды первой гармоники тока небаланса к приведенной амплитуде гармонической составляющей сквозного тока
    ...
    id: float
    id_ots: float #уставка дифференциальной отсечки
    index_ct: int | None = None
    id_alarm: int | None = None #контроль дифтока в % на сигнал
    t: float | None = None
    t_note: str  = ''
    time_prot: bool = True
    ...
    def calc_ust(self, te: TextEngine, res_sc_min: list, res_sc_max: list):
        ...
        f_knb1 = r'K_{\text{нб(1)}}'
        f_ikzmax = r'I_{\text{скв.макс}}'
        te.table_row('Ток срабатывания дифференциальной отсечки по условию отстройки от небаланса при внешнем КЗ',
                     te.m(r'I_{d*\text{отс.расч}} = \frac{' + f_kots + f_knb1 + f_ikzmax + r'}{I_{\text{ВН}}}'),
                     f'{self.Kots * self.Knb1 * self.i_kz_max / i_hv:.2f}')
        te.table_row('Коэффициент отстройки ', te.m(f_kots), 1.2)
        te.table_row('Отношение амплитуды первой гармоники тока небаланса к приведенной амплитуде гарминической '
                     'составляющей сквозного тока', te.m(f_knb1), self.Knb1)
        te.table_row(f'Максимальный сквозной ток короткого замыкания приведенный с стороне высокого напряжения '
                     f'{self.i_kz_max_note}', te.m(f_ikzmax), self.i_kz_max)
        te.table_row('Ток срабатывания дифференциальной отсечки по условию отстройки от броска тока намагничивания',
                     te.m(r'I_{d*\text{отс.расч}} \geq 5.0'), 5.0)
        te.table_row('Принимаем ток срабатывания дифференциальной отсечки', te.m(r'I_{d*\text{отс.}}'), self.id_ots)
        te.table_row('Уставка блокировки при броске тока намагничивания принимается, %', 'I2/I1Ratio', 14)
        te.table_row('Уставка блокировки при при перевозбуждении сердечника трансформатора, %', 'I5/I1Ratio', 25)
        te.table_row('Контроль дифференциального тока', 'IDiffAlarm', f'{self.id_alarm} %')
        te.table_row('Поперечная блокировка', 'CrossBlock', 'Off')
        te.table_row('Включение на повреждение', 'SOFTMode', 'Off')
        te.table_row('Исключение токов нулевой последовательности в обмотке высокого напряжения', 'ZSCurrSubtr', 'On')
        te.table_row('Исключение токов нулевой последовательности в обмотке среднего напряжения', 'ZSCurrSubtr', 'Off')
        te.table_row('Исключение токов нулевой последовательности в обмотке низкого напряжения', 'ZSCurrSubtr', 'Off')
